...
import ru.nekit.android.qls.quest.view.mediator.answer.IButtonsQuestAnswerMediator;
import ru.nekit.android.qls.quest.view.mediator.answer.IQuestAnswerMediator;
import ru.nekit.android.qls.quest.view.mediator.answer.ISpecialQuestAnswerMediator;
import ru.nekit.android.qls.quest.view.mediator.content.EmptyQuestContentMediator;
import ru.nekit.android.qls.quest.view.mediator.content.IQuestContentMediator;
import ru.nekit.android.qls.quest.view.mediator.title.IQuestTitleMediator;
...
import static ru.nekit.android.qls.quest.QuestContextEvent.EVENT_ERROR_ANSWER;
...
public class QuestMediatorFacade implements View.OnClickListener, IQuestMediatorFacade,
    ...
    public void onCreate(@NonNull QuestContext questContext) {
        ...
        if (defaultAnswerInputVisible) {
            mViewHolder.defaultAnswerInput.setVisibility(defaultAnswerInputVisible ? VISIBLE : GONE);
            boolean showAnswerButton = !answerButtonIsPresent() && questContext.answerButtonVisible();
            mViewHolder.defaultAnswerButton.setVisibility(showAnswerButton ? VISIBLE : GONE);
            if (!defaultAnswerInputVisible && showAnswerButton) {
                LinearLayout.LayoutParams answerButtonLayoutParameters =
                        (LinearLayout.LayoutParams) mViewHolder.defaultAnswerButton.getLayoutParams();
                answerButtonLayoutParameters.weight = 1;
                mViewHolder.defaultAnswerButton.setLayoutParams(answerButtonLayoutParameters);
            }
            mViewHolder.alternativeAnswerContainer.setVisibility(answerButtonIsPresent() ? VISIBLE : GONE);
            if (mViewHolder.defaultAnswerButton.getVisibility() == VISIBLE) {
                mViewHolder.defaultAnswerButton.setOnClickListener(this);
            }
            ...
            if (answerInputIsPresent()) {
                ...
                answerInput = mViewHolder.defaultAnswerInput;
            }
            if (answerInput != null) {
    ...
    public boolean onAnswer(@NonNull AnswerType answerType) {
        ...
        showRightAnswerWindow = showRightAnswerWindow && mTitleMediator.onAnswer(answerType);
        showRightAnswerWindow = showRightAnswerWindow && mContentMediator.onAnswer(answerType);
        showRightAnswerWindow = showRightAnswerWindow && mAnswerMediator.onAnswer(answerType);
    ...
    private void answerFunction() {
        if (!answerButtonIsPresent()) {
            CharSequence answerCharSequence = (answerInputIsPresent() ?
                    mContentMediator.getAnswerInput() : mViewHolder.defaultAnswerInput).getText();
    ...
    public void deactivate() {
        ...
        mViewHolder.defaultAnswerInput.setOnEditorActionListener(null);
    ...
    private void requestFocus() {
        if (questDelayedPlay() && !mQuestContext.questHasState(PLAYED)) {
            KeyboardHost.hideKeyboard(mQuestContext, mViewHolder.defaultAnswerInput, null);
        } else {
            if (answerButtonIsPresent()) {
                KeyboardHost.hideKeyboard(mQuestContext, mViewHolder.defaultAnswerInput);
            }
            if (mContentMediator.getAnswerInput() == null) {
                if (mViewHolder.defaultAnswerInput.getVisibility() == VISIBLE) {
                    KeyboardHost.showKeyboard(mQuestContext, mViewHolder.defaultAnswerInput);
                } else {
                    KeyboardHost.hideKeyboard(mQuestContext, mViewHolder.defaultAnswerInput);
                }
            } else {
    ...
    public void onEvent(@NonNull Intent intent) {
        ...
        switch (action) {
            ...
            case EVENT_ERROR_ANSWER:

                onAnswer(AnswerType.ERROR);
                Vibrate.make(mQuestContext, 400);
    ...
    static class QuestViewHolder extends ViewHolder {
        ...
        @NonNull
        final View defaultAnswerButton;
        @NonNull
        ...
        QuestViewHolder(@NonNull android.content.Context context) {
            ...
            answerContainer = (ViewGroup) view.findViewById(R.id.container_answer);
            contentContainer = (ViewGroup) view.findViewById(R.id.container_content);
            alternativeAnswerContainer = (ViewGroup) view.findViewById(R.id.container_answer_alternative);
            defaultAnswerInputContainer = (ViewGroup) view.findViewById(R.id.container_answer_default);
            defaultAnswerButton = view.findViewById(R.id.btn_answer_default);
            defaultAnswerInput = (EditText) view.findViewById(R.id.input_answer_default);
        }
    }
}