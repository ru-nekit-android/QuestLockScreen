...
public class LockScreen {
    ...
    //support
    public static boolean tryToLogin(@NonNull String unlockSecret) {
        boolean result = getUnlockSecret().equals(unlockSecret);
        if (result) {
            for (SessionType sessionType : SessionType.values()) {
                Session.startSession(sessionType);
            }
        }
        return result;
    }

    public static String getUnlockSecret() {
        return PreferencesUtil.getString(UNLOCK_SECRET);
    }

    public static void setUnlockSecret(@NonNull String unlockSecret) {
