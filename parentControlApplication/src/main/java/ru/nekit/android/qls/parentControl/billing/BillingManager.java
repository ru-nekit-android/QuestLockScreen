...
public class BillingManager implements PurchasesUpdatedListener {
     ...
     */
    private static final String BASE_64_ENCODED_PUBLIC_KEY = "";
    ...
    private void onQueryPurchasesFinished(PurchasesResult result) {
        ...
        if (mBillingClient == null || result.getResponseCode() != OK) {
        ...
        mPurchases.clear();
        onPurchasesUpdated(OK, result.getPurchasesList());
    }
    ...
    private void queryPurchases() {
        Runnable queryToExecute = new Runnable() {
            @Override
            public void run() {
                PurchasesResult purchasesResult = mBillingClient.queryPurchases(INAPP);
                // If there are subscriptions supported, we add subscription rows as well
                if (areSubscriptionsSupported()) {
                    PurchasesResult subscriptionResult = mBillingClient.queryPurchases(SUBS);
                    if (subscriptionResult.getResponseCode() == OK) {
                        purchasesResult.getPurchasesList().addAll(
                                subscriptionResult.getPurchasesList());
                    } else {
                        Log.e(TAG, "Got an error response trying to query subscription purchases");
                    }
                } else if (purchasesResult.getResponseCode() == OK) {
